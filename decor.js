// Décor Page JavaScript
document.addEventListener('DOMContentLoaded', function() {
    initializeDecorPage();
});

function initializeDecorPage() {
    setupDecorEventListeners();
    renderDecorProducts();
    updateCartUI();
    updateWishlistUI();
    setupScrollEffects();
...
function setupDecorEventListeners() {
...
function renderDecorProducts(productsToRender = null) {
    const decorProducts = productsToRender || products.filter(product => product.category === 'decor' || product.category === 'lighting');
    ...
    if (productsCount) {
        productsCount.textContent = `${decorProducts.length} Items`;
    }
...
function filterDecorProducts(filter) {
    const decorProducts = products.filter(product => product.category === 'decor' || product.category === 'lighting');
    
    let filteredProducts;
    if (filter === 'all') {
        filteredProducts = decorProducts;
    } else {
        filteredProducts = decorProducts.filter(product => product.subcategory === filter);
    }
    
    renderDecorProducts(filteredProducts);
}
...
function handleFilterChange() {
    ...
    if (selectedCategory) {
        filterDecorProducts(selectedCategory);
    }
}
...
function handleSortChange(e) {
    ...
    const decorProducts = products.filter(product => product.category === 'decor' || product.category === 'lighting');
    let sortedProducts = [...decorProducts];
    ...
    renderDecorProducts(sortedProducts);
}
...
function toggleWishlistItem(productId) {
    ...
    updateWishlistUI();
    renderDecorProducts(); // Re-render to update heart icons
}
